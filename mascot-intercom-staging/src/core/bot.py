...
class OnboardingBot:
    ...
    async def process_message(
        ...
        try:
            ...
            logger.info(f"🧠 Calling OpenAI Responses API ({config.openai_model})...")
            ...
            response = await self.openai_client.responses.create(
                ...
                text={"verbosity": "medium"},  # Medium verbosity for detailed responses
            )
            ...
            # ================================================================
            logger.info("=" * 80)
            logger.info("🔍 FULL RESPONSES API RESPONSE (for debugging):")
            logger.info(f"Response type: {type(response)}")
            logger.info(f"Response object: {response}")
            logger.info("=" * 80)

            # Extract the response text from the Responses API structure
            try:
                message_output = None
                ...
                if message_output and message_output.content:
                    ai_response = message_output.content[0].text
                    logger.info(f"✅ Extracted response: {ai_response[:200]}...")
                else:
                    logger.error("❌ Could not find message in response output")
                    ai_response = "I encountered an error extracting the response."

            except Exception as e:
                logger.error(f"❌ Error extracting response: {e}", exc_info=True)
                ai_response = "I encountered an error generating a response."
            ...
            if hasattr(response, "usage") and response.usage:
                # Ensure we always get integers (handle None values from API)
